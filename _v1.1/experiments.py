...
def rolling_train(data, window_size=504, train_size=252, val_size=252, step=63, gap=1, epochs=10, early_stopping=10, min=int(240/10), rollback=20):
    ...
    while i <= max_index:
        ...
        model = AlphaNetV1(num_features=num_features, min=min, rolling=rollback)
        ...
        i += step
    res.to_csv(f'./test/Factors_{res.index.get_level_values("date")[0]}-{res.index.get_level_values("date")[-1]}.xlsx')
