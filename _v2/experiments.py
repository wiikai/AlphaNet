...
def rolling_train(data, window_size=504, train_size=252, val_size=252, step=63, gap=1, epochs=20, early_stopping=5):
    levels = data.index.get_level_values('date').unique()
    num_features = len(data.columns) -1
    max_index = len(levels) - window_size - gap * 2 - step
    i = 0
    res = pd.DataFrame()
    ...
    while i <= max_index:
        ...
        window_dates = levels[i:i + window_size + gap * 2 + step]
        train_dates = window_dates[:train_size]
        val_dates = window_dates[train_size + gap:train_size + val_size + gap ]
        test_dates = window_dates[train_size + val_size + gap * 2 :train_size + val_size + gap * 2 + step]

        train_data = data.loc[train_dates]
        val_data = data.loc[val_dates]
        test_data = data.loc[test_dates]

        train_dataloader = create_dynamic_dataloader(train_data, shuffle=True)
        val_dataloader = create_dynamic_dataloader(val_data, shuffle=True)
        test_dataloader = create_dynamic_dataloader(test_data, shuffle=False)
        ...
        model = AlphaNetV2(num_features=num_features)
        ...
        with torch.no_grad():
            for idx, (images, labels) in enumerate(test_dataloader):
                ...
                predictions.loc[test_dates[-step:][idx]] = outputs
...
def main():
    set_seed(0)
    stop = '20240901'
    rollback = get_previous_trading_date(stop, 504 + 63*1 + 1*2)
